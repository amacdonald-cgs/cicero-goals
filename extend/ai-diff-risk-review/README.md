...
This extension is local-first. It helps a Cicero Goals run inspect the original request, board receipts, changed files, verification evidence, and known risk areas so the PM can produce a concise review brief for a human reviewer. It does not approve code, post comments, or call an external model by default.
