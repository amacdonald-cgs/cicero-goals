...
const DEFAULT_PUBLIC_HOST = "cicero-goals.localhost";
...
export async function main() {
  ...
  if (options.once) {
    if (options.json) {
      ...
      console.log(`Generated Cicero Goals board app at ${appDir}`);
    }
  ...
  if (options.json) {
    ...
    console.log(`Cicero Goals local board: ${server.url}`);
    console.log(`Cicero Goals local hub: ${server.hubUrl}`);
    if (server.registered) {
      console.log("Registered with the existing Cicero Goals local board hub.");
    } else {
...
async function registerWithBoardHub({ goalDir, host, port }) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404) {
      throw new Error(`Port ${port} is already in use, but it is not the Cicero Goals multi-board hub. Stop the existing local board process on ${host}:${port}, then retry.`);
    }
    throw new Error(`Cicero Goals local board hub rejected ${goalDir}: ${message}`);
  }
...
function redirectToFirstBoard(response, boards, baseUrl, settings = {}) {
  ...
  if (!board) {
    ...
    response.end("No Cicero Goals boards are registered.");
    return;
  }
...
function safePayload(goalDir) {
  try {
    ...
    return {
      ...
      error: error.message,
      goal: { title: "Cicero Goals Board", slug: "", status: "error", activeTask: "", tranche: "" },
      columns: [
...
function settingsPath() {
  return process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH
    || process.env.GOALBUDDY_LOCAL_BOARD_SETTINGS_PATH
    || join(homedir(), ".cicero-goals", "local-board-settings.json");
}

function usage() {
  console.log(`Cicero Goals Local Goal Board
...
Options:
  ...
  --host <host>   Local server bind host. Default: 127.0.0.1, advertised as cicero-goals.localhost.
  --port <port>   Local server port. Default: 41737 shared board hub.
  --once          Generate .cicero-goals-board and exit.
