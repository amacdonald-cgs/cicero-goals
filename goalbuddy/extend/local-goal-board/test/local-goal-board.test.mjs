...
test("writes a minimal Cicero Goals web app into the goal directory", () => {
  ...
  const logo = readFileSync(join(appDir, "cicero-goals-mark.png"));

  assert.match(html, /cicero-goals-mark\.png/);
  ...
  assert.match(js, /fetch\("https:\/\/api\.github\.com\/repos\/amacdonald-cgs\/cicero-goals"/);
  assert.match(js, /cicero-goals\.localBoardSettings\.v1/);
...
test("serves global local board settings with defensive normalization", async () => {
  const root = mkdtempSync(join(tmpdir(), "cicero-goals-local-board-settings-"));
  const goalDir = join(root, "settings-goal");
  const settingsPath = join(root, "settings.json");
  const previousSettingsPath = process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH;
  try {
    process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH = settingsPath;
    mkdirSync(join(goalDir, "notes"), { recursive: true });
    ...
    if (previousSettingsPath === undefined) {
      delete process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH;
    } else {
      process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH = previousSettingsPath;
    }
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("parses CLI options", () => {
  ...
  assert.equal(parseArgs(["--goal", "docs/goals/demo"]).publicHost, "cicero-goals.localhost");
  assert.deepEqual(parseArgs(["--goal", "docs/goals/demo", "--port", "0", "--once", "--json"]), {
    goal: "docs/goals/demo",
    host: "127.0.0.1",
    publicHost: "cicero-goals.localhost",
    port: 0,
    once: true,
    json: true,
  });
...
test("advertises cicero-goals.localhost while binding to loopback", async () => {
  const root = mkdtempSync(join(tmpdir(), "cicero-goals-local-board-public-host-"));
  ...
  try {
    ...
    try {
      ...
      assert.equal(url.hostname, "cicero-goals.localhost");
...
test("runs when installed under a symlinked temp path", () => {
  const root = mkdtempSync(join(tmpdir(), "cicero-goals-local-board-direct-"));
  try {
...
test("serves board JSON and streams live state changes over SSE", async () => {
  const root = mkdtempSync(join(tmpdir(), "cicero-goals-local-board-"));
...
test("streams parent board updates when linked child subgoal state changes", async () => {
  const root = mkdtempSync(join(tmpdir(), "cicero-goals-local-board-subgoal-live-"));
...
test("serves multiple local boards from one shared hub URL", async () => {
  const root = mkdtempSync(join(tmpdir(), "cicero-goals-local-board-hub-"));
  const firstGoalDir = join(root, "first-goal");
  const secondGoalDir = join(root, "second-goal");
  const previousSettingsPath = process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH;
  try {
    process.env.CICERO_GOALS_LOCAL_BOARD_SETTINGS_PATH = join(root, "hub-settings.json");
