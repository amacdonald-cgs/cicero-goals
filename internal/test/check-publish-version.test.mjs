...
test("publish version check passes when package version is newer than npm", () => {
  ...
  assert.match(result.stdout, new RegExp(`${escapeRegex(pkg.name)}@${escapeRegex(currentVersion)} > published ${escapeRegex(previousVersion)}`));
});
...
test("publish version check rejects already-published package version", () => {
  ...
  assert.match(result.stderr, new RegExp(`${escapeRegex(pkg.name)}@${escapeRegex(currentVersion)} has already been published`));
});
